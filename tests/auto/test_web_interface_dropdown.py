#!/usr/bin/env python3
"""
Test Web Interface Dropdown Issue
Creates a simple HTML test to diagnose why the "Specific Model" dropdown is greyed out.
"""
...
def create_diagnostic_html():
    """Create a diagnostic HTML file to test the dropdown issue."""
...
<body>
    <h1>🔧 Dropdown Diagnostic Test</h1>
    ...
    <script>
        ...
        function loadModels(provider, modelSelectId, resultId) {
            ...
            if (!provider) {
                ...
                if (resultDiv) resultDiv.innerHTML = '<span class="info">ℹ️ Please select a provider first</span>';
                return;
            }
            ...
            if (!models) {
                ...
                modelSelect.disabled = true;
                if (resultDiv) resultDiv.innerHTML = '<span class="error">❌ No models found for ' + provider + '</span>';
                return;
            }
            ...
            if (resultDiv) {
                resultDiv.innerHTML = `<span class="success">✅ Successfully loaded ${modelCount} models for ${provider}</span>`;
            }
        }
        ...
        function inspectModelData() {
            ...
            if (typeof modelData === 'undefined') {
                ...
                html += `📊 Providers: ${Object.keys(modelData).length}<br>`;
                
                Object.entries(modelData).forEach(([provider, models]) => {
                    html += `🔹 ${provider}: ${Object.keys(models).length} models<br>`;
                });
            }
        ...
        function testEventListeners() {
            ...
            html += `Provider change listener: ${hasChangeListener ? '✅ EXISTS' : '❌ MISSING'}<br>`;
            ...
            setTimeout(() => {
                ...
                html += `Model dropdown enabled after change: ${isEnabled ? '✅ YES' : '❌ NO'}<br>`;
                html += '</div>';
                resultDiv.innerHTML = html;
            }, 500);
        }
    ...
    diagnostic_file = "tests/auto/dropdown_diagnostic.html"
...
def main():
    """Create diagnostic HTML and provide instructions."""
    print("🔧 Creating Web Interface Dropdown Diagnostic Test")
    print("=" * 60)
    
    # Create the diagnostic HTML
    diagnostic_file = create_diagnostic_html()
    
    print(f"✅ Created diagnostic test: {diagnostic_file}")
    print(f"📍 Full path: {os.path.abspath(diagnostic_file)}")
    
    print("\n📋 To test the dropdown issue:")
    print("1. Open the diagnostic file in your web browser:")
    print(f"   file://{os.path.abspath(diagnostic_file)}")
    print("2. Click 'Run Diagnostic Test' to test all providers")
    print("3. Check the debug output for any errors")
    print("4. Compare with the main interface behavior")
    
    print("\n🔍 The diagnostic will test:")
    print("- Basic dropdown functionality")
    print("- Model data loading")
    print("- Event listener setup")
    print("- JavaScript error detection")
    
    print(f"\n💡 This will help identify why the dropdown is greyed out in the main interface.")
